...
function do_drop ()
{
    ...
    "${GPG_BIN}" --no-greeting --no-auto-check-trustdb -q --batch --no-tty \
    --cipher-algo AES256 --compress-level 0 --output "${WORKSPACE}/${REMOTE_FILE}" \
    --recipient "${6}" --encrypt "${1}" || exit ${E_ENCRYPT}
